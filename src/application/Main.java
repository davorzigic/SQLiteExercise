...
public class Main extends Application {
	...
	ResultSet rs = null;
	TextField id, fn, ln, age, un, queryField;
	PasswordField pass;
	DatePicker date;
	...
	public void start(Stage primaryStage) {
		try {
			...
			Scene newScene = new Scene(layout, 1200, 600, Color.rgb(0, 0, 0));
			...
			// Getting the age from the database and inserting it into the
			// fourth column
			...
			// Getting the username from the database and inserting it into the
			// fifth column
			...
			// Getting the password from the database and inserting it into the
			// sixth column
			...
			load.setOnAction(e -> {
				try {
					...
					while (rs.next()) {
						data.add(new User(rs.getString("id"), rs.getString("firstName"), rs.getString("lastName"),
								rs.getString("age"), rs.getString("username"), rs.getString("password"),
								rs.getString("DOB")));
						table.setItems(data);
					}
					column1.sortableProperty();
					pst.close();
					rs.close();
				} catch (Exception e2) {
			...
			queryField.setFont(Font.font("SanSerif", 20));
			queryField.setPromptText("Enter query here");
			queryField.setMaxWidth(600);

			Button filter = new Button("Filter");
			filter.setFont(Font.font("SanSerif", 15));
			filter.setOnAction(e -> {
				try {
					if (queryField.getText().isEmpty()) {
						...
						alert.setTitle("Information dialog");
						alert.setHeaderText(null);
						alert.setContentText("You have to enter query.");
						alert.showAndWait();
					} else {
			...
			rightSide.getChildren().addAll(table, queryField, filter);
			layout.setRight(rightSide);
			BorderPane.setMargin(table, new Insets(0, 20, 10, 0));

			HBox hBox = new HBox(5);
			hBox.getChildren().addAll(load);
